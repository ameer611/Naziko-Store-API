...
async def get_customers(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user:
        raise HTTPException(status_code=401, detail="Unauthorized")
    if not user["is_superuser"]:
        raise HTTPException(status_code=403, detail="Forbidden")

    customers = get_customers_from_db(db, user)
    return customers


@router.get("/customers/{customer_id}", response_model=UserResponse, status_code=200)
async def get_customer_by_id(customer_id: int, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user:
        raise HTTPException(status_code=401, detail="Unauthorized")
    if not user["is_superuser"]:
        raise HTTPException(status_code=403, detail="Forbidden")

    customer = get_customer_by_id_from_db(db, customer_id)
    if not customer:
...
async def change_customer_password(customer_id: int, new_password: SecretStr, user: dict = Depends(get_current_user),
    ...
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    changed_customer = change_customer_password_on_db(db, customer, new_password.get_secret_value())
...
async def delete_customer_by_id(customer_id: int, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if not user["is_superuser"]:
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    message = delete_customer_from_db(db, customer)
    return message
