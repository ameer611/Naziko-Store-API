...
async def create_percentage(percentage: PercentageCreate,
    ...
    percentage = await create_percentage_on_db(db, percentage)
    if not percentage:
...
async def get_percentages(current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Get all percentages"""
    if not current_user:
        raise HTTPException(status_code=401, detail="You need to be logged in to perform this action.")
    ...
    percentages = await get_percentages_from_db(db)
...
async def update_percentage(percentage_id: int, percentage: PercentageUpdate, current_user=Depends(get_current_user),
    ...
    message = await update_percentage_on_db(percentage_id, db, percentage)
    if not message:
