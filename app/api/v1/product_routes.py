...
from app.alibaba_scraper.products_scraper import get_product_by_link_from_website, get_product_variants, \
...
from app.crud.product import get_products_from_db, get_product_from_db_if_exists, save_product_on_db, \
...
async def get_products(db: Session = Depends(get_db)):
    products = get_products_from_db(db)
    return products


@router.post("/product-details", response_model=OnlineProductResponse)
async def get_product(product_link: ProductLink, user=Depends(get_current_user), db: Session = Depends(get_db)):
    if not user:
        raise HTTPException(status_code=401, detail="You need to be logged in to perform this action.")
    product_db = get_product_from_db_if_exists(db, product_link.product_link)
    if product_db:
        return product_db
    product = get_product_by_link_from_website(product_link.product_link)
    if not product:
        return {"message": "Product not found."}
    saved_product = save_product_on_db(db, product, product_link.product_link)
...
async def get_product(product_link: ProductLink, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    variants = get_product_variants(product_link.product_link)
    if not variants:
        return {"message": "No variants found."}
    variants_db = update_product_add_variants_to_db_product(db, variants, product_link.product_link)
    if not variants_db:
        raise HTTPException(status_code=404, detail="Product not found")
    return variants_db
...
async def search_product_by_title(product_name: str, page_number: int = 1, user=Depends(get_current_user)):
    ...
    products = search_product_by_name_from_website(product_name, page=page_number)
    return products
...
async def search_product_by_image(file: UploadFile = File(...), user=Depends(get_current_user)):
    ...
    products = search_product_by_image_from_website(file_path)
    response = delete_file_from_server(file_path)
    if products:
        return products
...
async def get_product_reviews(product_link: ProductLink, user=Depends(get_current_user)):
    reviews = get_product_comments(product_link.product_link)
    if not reviews:
